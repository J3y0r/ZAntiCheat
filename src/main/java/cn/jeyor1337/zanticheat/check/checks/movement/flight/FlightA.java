...
public class FlightA extends MovementCheck implements Listener {
    ...
    public void onAsyncMovement(ZACAsyncPlayerMoveEvent event) {
        ...
        int slowFallingEffectAmplifier = getEffectAmplifier(zacPlayer.cache, PotionEffectType.JUMP);
    ...
    public void beforeMovement(ZACAsyncPlayerMoveEvent event) {
        ...
        for (int i = 0; i < 6 && i < HistoryElement.values().length; i++) {
            Location location = zacPlayer.cache.history.onEvent.location.get(HistoryElement.values()[i]);
            if (previous == null) {
                first = location;
                previous = location;
                continue;
