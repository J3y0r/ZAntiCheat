...
public class FlightC extends MovementCheck implements Listener {
        ...
        Scheduler.runTask(true, () -> {
            ...
            if (!isBedrockPlayer) {
                if (System.currentTimeMillis() - buffer.getLong("lastGlidingLagPossibleTime") < 5 * 1000)
                    callViolationEventIfRepeat(player, zacPlayer, event, buffer, 300);
                else
                    callViolationEventIfRepeat(player, zacPlayer, event, buffer, 750);
            } else {
                if (System.currentTimeMillis() - buffer.getLong("lastGlidingLagPossibleTime") < 5 * 1000)
                    callViolationEventIfRepeat(player, zacPlayer, event, buffer, 250);
                else
                    callViolationEventIfRepeat(player, zacPlayer, event, buffer, 400);
            }
        });
    }
    ...
    private static boolean isSpeedDecreasing(PlayerCacheHistory<Location> history) {
        ...
        boolean legacy = VerIdentifier.getVersion() == ZACVersion.V1_8;
