...
public class SpeedA extends MovementCheck implements Listener {
    ...
    @EventHandler
    public void totalHorizontal(ZACAsyncPlayerMoveEvent event) {
    ...
    @EventHandler
    public void airHorizontal(ZACAsyncPlayerMoveEvent event) {
        ZACPlayer zacPlayer = event.getZacPlayer();
        PlayerCache cache = zacPlayer.cache;
        Player player = event.getPlayer();
        Buffer buffer = getBuffer(player, true);
        ...
        if (zacPlayer.violations.getViolations(getCheckSetting(this).name) == 0 &&
            ...
            if (CooldownUtil.isSkip(160, zacPlayer.cooldown, this))
                return;
        ...
        int justBoostEffectAmplifier = getEffectAmplifier(zacPlayer.cache, PotionEffectType.JUMP);
    ...
    @EventHandler(priority = EventPriority.LOW)
    public void beforeAirMovement(ZACAsyncPlayerMoveEvent event) {
        ZACPlayer zacPlayer = event.getZacPlayer();
        Player player = event.getPlayer();
