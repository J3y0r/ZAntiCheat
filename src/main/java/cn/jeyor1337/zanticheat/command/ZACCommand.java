package cn.jeyor1337.zanticheat.command;
...
import cn.jeyor1337.zanticheat.util.player.brand.ClientBrandRecognizer;
...
public class ZACCommand implements TabExecutor {

    private static final Placeholder[] SERVER_PLACEHOLDERS = new Placeholder[]{Placeholder.PREFIX, Placeholder.VERSION,
            Placeholder.TPS};

    private static boolean isReportCommand(Command command) {
        return command.getName().equalsIgnoreCase("report");
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        if (isReportCommand(command)) {
            if (!ACPermission.hasPermission(sender, ACPermission.REPORT) && !ACPermission.hasPermission(sender, ACPermission.ALL)) {
                sender.sendMessage(getNoPermissionError(hex));
                return true;
            }
            if (args.length != 1) {
                sender.sendMessage(getInvalidFormatError("/" + label + " <player>", hex));
                return true;
            }
            ...
            ApiUtil.reportPlayer(player);
            String message = ConfigManager.Config.Messages.CommandMessages.Report.message;
        ...
        if (args.length > 0) {
            ...
            if (args[0].equalsIgnoreCase("client")) {
                if (!ACPermission.hasPermission(sender, ACPermission.CLIENT) && !ACPermission.hasPermission(sender, ACPermission.ALL)) {
                    sender.sendMessage(getNoPermissionError(hex));
                    return true;
                }
                if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("cps")) {
                if (!ACPermission.hasPermission(sender, ACPermission.CPS) && !ACPermission.hasPermission(sender, ACPermission.ALL)) {
                    sender.sendMessage(getNoPermissionError(hex));
                    return true;
                }
                if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("reload")) {
                if (!ACPermission.hasPermission(sender, ACPermission.RELOAD) && !ACPermission.hasPermission(sender, ACPermission.ALL)) {
                    sender.sendMessage(getNoPermissionError(hex));
                    return true;
                }
                ...
                Main.getInstance().reloadDatabaseSubsystem();
                Main.getInstance().reloadVelocitySupportSubsystem();
            ...
            if (args[0].equalsIgnoreCase("checks")) {
                if (!ACPermission.hasPermission(sender, ACPermission.CHECKS) && !ACPermission.hasPermission(sender, ACPermission.ALL)) {
                    sender.sendMessage(getNoPermissionError(hex));
                    return true;
                }
            ...
            if (args[0].equalsIgnoreCase("tps")) {
                if (!ACPermission.hasPermission(sender, ACPermission.TPS) && !ACPermission.hasPermission(sender, ACPermission.ALL)) {
                    sender.sendMessage(getNoPermissionError(hex));
                    return true;
                }
            ...
            if (args[0].equalsIgnoreCase("ping")) {
                if (!ACPermission.hasPermission(sender, ACPermission.PING) && !ACPermission.hasPermission(sender, ACPermission.ALL)) {
                    sender.sendMessage(getNoPermissionError(hex));
                    return true;
                }
                if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("alerts")) {
                if (!ACPermission.hasPermission(sender, ACPermission.ALERTS_TOGGLE) && !ACPermission.hasPermission(sender, ACPermission.ALERTS) &&
                        !ACPermission.hasPermission(sender, ACPermission.ALL)) {
                    sender.sendMessage(getNoPermissionError(hex));
                    return true;
                }
                ...
                zacPlayer.cache.alerts = !zacPlayer.cache.alerts;

                String message = zacPlayer.cache.alerts ?
            ...
            if (args[0].equalsIgnoreCase("teleport")) {
                if (!ACPermission.hasPermission(sender, ACPermission.ALERTS_TELEPORT) && !ACPermission.hasPermission(sender, ACPermission.ALERTS) &&
                        !ACPermission.hasPermission(sender, ACPermission.ALL)) {
                    sender.sendMessage(getNoPermissionError(hex));
                    return true;
                }
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        if (isReportCommand(command)) {
            if (args.length == 1)
                return getOnlinePlayerNames().stream().filter(name -> name.startsWith(args[0])).collect(Collectors.toList());
            return Collections.emptyList();
        }
