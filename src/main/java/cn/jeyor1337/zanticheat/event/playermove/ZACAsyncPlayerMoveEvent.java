...
import cn.jeyor1337.zanticheat.event.playermove.blockcache.BlockCache;
...
public class ZACAsyncPlayerMoveEvent extends Event implements Cancellable {
    ...
    public ZACAsyncPlayerMoveEvent(ZACPlayerMoveEvent event) {
        ...
        this.fromBlockCache = zacPlayer.cache.fromBlockCache;
        if (FoliaUtil.isStable(event.getPlayer())) {
            this.toBlockCache = new BlockCache(player, to);
            isPlayerClimbing = zacPlayer.isClimbing();
            isPlayerInWater = zacPlayer.isInWater();
        } else {
            toBlockCache = fromBlockCache;
            isPlayerClimbing = false;
            isPlayerInWater = false;
        }
        zacPlayer.cache.fromBlockCache = this.toBlockCache;
    }
