...
public class ApiUtil {
    ...
    public static final Set<UUID> REPORTED_PLAYERS = ConcurrentHashMap.newKeySet();
    ...
    public static boolean isReported(Player player) {
        return player != null && REPORTED_PLAYERS.contains(player.getUniqueId());
    }

    public static void reportPlayer(Player player) {
        if (player == null)
            return;
        REPORTED_PLAYERS.add(player.getUniqueId());
    }

    public static void unreportPlayer(Player player) {
        if (player == null)
            return;
        REPORTED_PLAYERS.remove(player.getUniqueId());
    }

    public static void clearReportedPlayers() {
