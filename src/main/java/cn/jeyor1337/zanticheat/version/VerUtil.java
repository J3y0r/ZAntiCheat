package cn.jeyor1337.zanticheat.version;
...
public class VerUtil {
    ...
    static {
        ZACVersion lacVersion = VerIdentifier.getVersion();
        switch (lacVersion) {
            case V1_8:
                multiVersion = new V1_8(Main.getInstance()) {
                    @Override
                    public void onBlockPlace(BlockPlaceEvent event) {
                        ZACEventCaller.callBlockPlaceEvents(event);
                    }

                    @Override
                    public void onBlockBreak(BlockBreakEvent event) {
                        ZACEventCaller.callBlockBreakEvents(event);
                    }

                    @Override
                    public void onMovement(PlayerMoveEvent event) {
                        ZACEventCaller.callMovementEvents(event);
                    }

                    @Override
                    public void onEntityDamage(EntityDamageByEntityEvent event) {
                        ZACEventCaller.callEntityDamageEvent(event);
                    }
                };
                break;
            case V1_9:
